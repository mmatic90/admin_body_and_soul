...
function overrideStatusLabel(value: string, isWorking: boolean) {
  ...
  return ["day_off", "vacation", "sick_leave"].includes(value) ? "Ne radi" : "Ne radi";
}
...
export async function getEmployeeSchedulePageData(employeeId: string): Promise<EmployeeSchedulePageData | null> {
  const supabase = await createClient();
  ...
  const [
    ...
    supabase.from("employees").select("id, display_name, color_hex").eq("id", employeeId).single(),
    supabase
      .from("employee_default_schedule")
      .select("id, employee_id, day_of_week, start_time, end_time, break_start_time, break_end_time, is_working")
      .eq("employee_id", employeeId)
      .order("day_of_week", { ascending: true }),
    supabase
      .from("employee_schedule_overrides")
      .select("id, employee_id, override_date, override_type, start_time, end_time, break_start_time, break_end_time, note")
      .eq("employee_id", employeeId)
  ...
  if (defaultScheduleError) throw new Error("Nije moguće dohvatiti default raspored.");
  if (overridesError) throw new Error("Nije moguće dohvatiti overrideove.");

  const defaultRows = defaultSchedule ?? [];
  const overrideRows = overrides ?? [];

  const upcomingSchedule: EmployeeUpcomingScheduleItem[] = Array.from({ length: 5 }, (_, index) => {
    ...
    if (override) {
      ...
      return {
        ...
        reason_label: override.note?.trim() ? override.note : overrideReasonLabel(override.override_type, isWorking),
        is_override: true,
      };
    }

    const dayOfWeek = currentDate.getDay();
    const defaultItem = defaultRows.find((row) => row.day_of_week === dayOfWeek);
