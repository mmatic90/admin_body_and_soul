    ...
    <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-3xl border border-app-soft bg-app-card p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-app-muted">Ukupno prijava</p>
        </div>
        <div className="rounded-3xl border border-app-soft bg-app-card p-5 shadow-sm">
        ...
        <div className="rounded-3xl border border-app-soft bg-app-card p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-app-muted">Spremno za čišćenje</p>
        </div>
      </div>
                ...
                {items.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="group transition hover:bg-app-card-alt">
                      <td className="p-0"><Link href={href} className="block px-5 py-4 font-medium text-app-text">{statusLabels[item.status] ?? item.status}</Link></td>
                      <td className="p-0"><Link href={href} className="block px-5 py-4 text-app-muted">{typeLabels[item.type] ?? item.type}</Link></td>
                      <td className="p-0"><Link href={href} className="block px-5 py-4 text-app-muted">{priorityLabels[item.priority] ?? item.priority}</Link></td>
                      <td className="p-0"><Link href={href} className="block px-5 py-4 text-app-muted">{item.created_by_name ?? item.created_by_email ?? "Korisnik"}</Link></td>
                      ...
                      <td className="whitespace-nowrap p-0"><Link href={href} className="block px-5 py-4 text-app-muted">{new Intl.DateTimeFormat("hr-HR", { dateStyle: "short", timeStyle: "short" }).format(new Date(item.created_at))}</Link></td>
                      <td className="p-0 text-right">
                        <Link href={href} className="inline-flex items-center gap-2 px-5 py-4 font-semibold text-app-accent transition hover:underline">
                          Otvori <ArrowRight className="h-4 w-4" />
                        </Link>
                      </td>
                    </tr>
                  );
